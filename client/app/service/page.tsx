    ...
    >
      <div style={{ position: "relative", height: "200px", overflow: "hidden" }}>
        ...
        <span style={{
          ...
          color: "white", fontSize: "11px", fontWeight: "600", padding: "4px 10px", borderRadius: "20px",
      ...
      <div style={{ padding: "16px" }}>
        ...
        <h3 style={{ margin: "4px 0 2px", fontSize: "15px", fontWeight: "700", color: "#111827", lineHeight: 1.3 }}>{trip.title}</h3>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "5px", marginBottom: "12px" }}>
          {trip.highlights.slice(0, 2).map(h => (
            <span key={h} style={{ fontSize: "10px", background: "#f3f4f6", color: "#374151", padding: "3px 8px", borderRadius: "20px", fontWeight: "500" }}>{h}</span>
          ))}
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <button style={{
            ...
            borderRadius: "10px", padding: "9px 18px", fontSize: "12px", fontWeight: "700",
            cursor: "pointer", letterSpacing: "0.03em", boxShadow: "0 4px 12px rgba(99,102,241,0.3)",
          }}>Book Now</button>
        </div>
      </div>
    </div>
  ...
  const displayed = useMemo(() => {
    let result = TRIPS.filter(trip => {
      ...
      if (searchQuery.trim()) {
        ...
        if (!trip.title.toLowerCase().includes(q) && !trip.destination.toLowerCase().includes(q) && !trip.country.toLowerCase().includes(q)) return false;
      }
      return true;
    });
    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .top-bar-right { display: flex; align-items: center; gap: 8px; flex: 1; min-width: 0; overflow: hidden; }
      ...
      <div style={{ minHeight: "100vh", background: "#f8fafc", fontFamily: "system-ui, sans-serif" }}>
        ...
        {/* ── Filter Drawer (fixed overlay, above everything) ── */}
        <div className={`drawer-panel${drawerOpen ? " open" : ""}`}>
          {/* Drawer header */}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
            <span style={{ fontSize: "15px", fontWeight: "800", color: "#111827" }}>Filters</span>
            <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
              {activeFilterCount > 0 && (
                <button onClick={clearAllFilters} style={{ background: "none", border: "none", color: "#ef4444", fontSize: "12px", fontWeight: "600", cursor: "pointer" }}>
          ...
          <FilterSection title="Budget (per person)">
            ...
            <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
              {([["Under ₹20K", 10000, 20000], ["₹20K–₹50K", 20000, 50000], ["₹50K–₹1L", 50000, 100000], ["₹1L+", 100000, 300000]] as [string, number, number][]).map(([label, min, max]) => (
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
              {GROUP_OPTIONS.map(({ id, label, emoji }) => (
                <button key={id} onClick={() => toggleGroup(id)}
                  ...
                  <span style={{ fontSize: "11px", fontWeight: "600", color: selectedGroups.includes(id) ? "#1d4ed8" : "#374151" }}>{label}</span>
                </button>
              ))}
            </div>
          </FilterSection>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              {DURATION_OPTIONS.map(({ label }) => (
                <label key={label} style={{ display: "flex", alignItems: "center", gap: "10px", cursor: "pointer" }}>
                  <div onClick={() => toggleDuration(label)}
                    style={{
        ...
        {/* ── Page Header ── */}
        <div style={{ padding: "32px 20px 20px", background: "linear-gradient(135deg,#020b18 0%,#0f2137 100%)" }}>
          <p style={{ color: "#e7d393", fontSize: "12px", fontWeight: "700", letterSpacing: "0.15em", textTransform: "uppercase", margin: "50px 0 8px" }}>
            ✈ Explore Packages
          </p>
          <h1 style={{ color: "white", fontSize: "clamp(1.4rem, 4vw, 2.6rem)", fontWeight: "900", margin: "0 0 6px", letterSpacing: "-0.5px" }}>
        ...
        <div style={{ padding: "20px 16px" }}>
          ...
          <div className="top-bar">
            ...
            <div className="top-bar-left">
              ...
              >
                ...
                {activeFilterCount > 0 && (
                  <span style={{
                    background: "#3b82f6", color: "white", fontSize: "10px", fontWeight: "800",
                    borderRadius: "20px", padding: "1px 7px", marginLeft: "2px",
                  }}>{activeFilterCount}</span>
                )}
              </button>
              <span style={{ fontSize: "13px", color: "#6b7280", whiteSpace: "nowrap" }}>
                <strong style={{ color: "#111827" }}>{displayed.length}</strong> trips
              </span>
            </div>

            {/* Right: sort pills */}
            <div className="top-bar-right">
              <span style={{ fontSize: "12px", color: "#6b7280", fontWeight: "600", whiteSpace: "nowrap" }}>Sort:</span>
          ...
          {displayed.length > 0 ? (
            ...
            <div style={{ textAlign: "center", padding: "80px 20px" }}>
              ...
              <h3 style={{ fontSize: "18px", fontWeight: "700", color: "#111827", margin: "0 0 8px" }}>No trips match your filters</h3>
              <p style={{ color: "#9ca3af", fontSize: "14px", margin: "0 0 20px" }}>Try adjusting your budget, duration, or travel type</p>
              <button onClick={clearAllFilters} style={{
                background: "#111827", color: "white", border: "none",
                borderRadius: "10px", padding: "10px 24px", fontSize: "13px", fontWeight: "700", cursor: "pointer",
